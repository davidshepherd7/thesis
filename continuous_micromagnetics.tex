...
By inverting \eqref{eq:42} we can obtain an ``effective field'' $\Hv$ for any energy term $E$:
\begin{equation}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
The boundary conditions are for \eqref{eq:Hms}
\begin{equation}
  \begin{aligned}
...
\begin{equation}
  ...
  \label{eq:Heff}
\end{equation}
Equation~\eqref{eq:38} represents the precession of the magnetisation about the effective field, as shown in \autoref{fig:LLG-terms}. Other effective fields can be added to equation~\eqref{eq:Heff} to model other physical effects such as finite temperature or magnetostriction.

Equation~\eqref{eq:38} describes an undamped precessional motion, \ie no energy is lost and the magnetisation continues to precess forever. This is obviously not the case for many problems of interest so an empirical damping term is added \cite{Landau1935}:
\begin{equation}
  \label{eq:LL}
  \dMdt = - \gymagc \Mv \times \Hv - \frac{\dampc_L \gymagc}{M_s} \Big( \Mv \times (\Mv \times \Hv) \Big),
\end{equation}
where $\dampc$ is an experimentally determined material parameter related to the strength of the damping. Equation~\eqref{eq:LL} is referred to as the Landau-Lifshitz equation.
...
Equations~\eqref{eq:LL}, \eqref{eq:Gilbert} and \eqref{eq:LLG} link the unknown magnetisation vector as a function of time, $\Mv(t)$, with the effective field, $\Hv$.
...
Also we know that the magnetisation length does not change,\footnote{Physically this is part of the definition of the problem. The proof that this fact is embodied in the LLG equation is given in \autoref{sec:prop-cont-llg}.} therefore $\pd{\Mv}{\av} \cdot \Mv = 0$ for any direction $\av$ (since the change in magnetisation projected onto the magnetisation direction is the change in magnetisation length). 
...
Similarly for equation~\eqref{eqn:phind} we have
\begin{align*}
...
\end{equation}
However this means that time scale changes (\ie the $t^*$s are different) when switching between the two forms \eqref{eq:ll-nd-simpler} and \eqref{eq:53}, making comparisons more difficult.
...
To see this we take the dot product of~\eqref{eq:llg-prop-form} with $\mv$ and use~\eqref{eq:dot-cross-id} to obtain
\begin{equation}
  \label{eq:56}
  \mv \cdot \dmdt = 0.
\end{equation}
...
The change in energy over time can be derived similarly to the change in magnetisation by taking the $\ltwo$ inner product\footnote{The $\ltwo$ inner product is given by $\ltip{\av}{\bv} = \int_\magd \av \cdot \bv \d\magd$.} of each side of~\eqref{eq:llg-prop-form} with $\hv - \dampc \dmdt$.
...
\begin{equation}
  ...
  \label{eq:64}
\end{equation}
Finally, substituting \eqref{eq:64} into \eqref{eq:58} leaves
\begin{equation}
