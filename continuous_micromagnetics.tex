...
The units of $\Mv$ are $\Mu$, the length of $\Mv(\xv,t)$ is fixed and given by the saturation magnetisation $M_s$, a material parameter.
...
We call the region of magnetic material $\magd$, it's boundary as $\boundd$ and the external domain as $\extd$.
...
The first thing that must be considered when modelling any physical system is the energy.
...
Each energy has a corresponding effective field, which is ??ds need some intuitive description...

We begin by providing a description of each of these energies.
We can also derive the effective fields caused by these energies using
\begin{equation}
...
This can be well approximated by a classical expression for exchange energy \cite{Aharoni1996} as
\begin{equation}
  \label{eq:39}
  \Eex =  \frac{\Exchc}{M_s^2} \int_{\magd} (\nabla M_x)^2  + (\nabla M_y)^2  + (\nabla M_z)^2 \d \magd,
\end{equation}
where $\Exchc$ is the material dependant exchange constant representing the strength of the exchange coupling.
...
Magnetic materials generate magnetic fields, which in turn affects the magnetic material emitting the field. 
...
\begin{equation}
  ...
  \label{eq:36}
\end{equation}
where $\ev$ is the easy axis and $K_u$ is the material dependant first order anisotropy energy density coefficient.\footnote{It is possible to define many different forms for this energy. Be warned that the choice of definition changes the meaning of $K_u$!}
The first order approximation is valid when the anisotropy energy density coefficients are such that $K_u = K_1 >> K_2$, which is the case in most materials \cite{Kronmuller2003}.
...
In this thesis we are concerned only with dynamic model, such models calculate the time evolution of the magnetisation.
...
Equation~\eqref{eq:38} describes an undamped precessional motion, \ie no energy is lost and the magnetisation continues to precess forever. This is obviously not the case so an empirical damping term is added:
\begin{equation}
  \label{eq:LL}
...
The boundary condition on the magnetisation $\mv$ (in dimensional form) is \cite[178, 181]{Aharoni1996}
\begin{equation}
  \Mv \times \bigb{ C \dMdn + \pd{w_s}{\Mv}} = \zerov,
\end{equation}
where $C$ is an exchange coefficient and $w_s$ is the surface anisotropy.

For simplicity we consider the case of zero surface anisotropy ($w_s \equiv 0$), in this case the boundary condition becomes
\begin{equation}
  \label{eq:llg-bc}
  \Mv \times \dMdn = \zerov.
\end{equation}
...
\subsection{High temperature effects}
\label{sec:magn-bound-cond}
??ds not sure if I need this

All equations so far in this Chapter are for the zero temperature case.

Moderate temperature effects (significantly below the Curie temperature) can modelled by adding a stochasitic term to the effective field, as done by \eg \cite{DAquino2006}.
...
In general it is useful to remove all unecessary parameters from a differential equation before attempting so solve it.
...
Finally, having all quantities a similar size helps to ensure the good behaviour of numerical methods.
...
For this purpose it is convinient to write the non-dimensional LLG equation in the form
\begin{equation}
...
Note that length conservation is a \emph{pointwise} property, \ie $\abs{\mv} = 1$ at every point in space. 
In constrast the energy decay/conservation is a \emph{global} property, \ie an integral over all space is conserved.
