...
We begin by providing a description of each of these effects and their contribution to the total free energy density, $F$. We can get the effective fields given by these energies by taking
\begin{equation}
  \Hv = \dfrac{-1}{\mu_0 V_{\magd}} \dfrac{\delta F}{\delta \Mv},
\end{equation}
where $\delta$ indicates a variational derivative and $V_{\magd}$ is the volume of the magnetic domain.

The energy and field expressions below are taken from Coey2010.\cite{Coey2010}

\subsubsection{Exchange Energy}

In a ferromagnetic material neighbouring magnetic moments prefer to align parallel to each other due to quantum mechanical effects. This can be approximated by a classical expression for exchange energy:\cite{Aharoni1996}
\begin{equation}
  \label{eq:39}
...
\begin{equation}
  ...
  \label{eq:36}
  % should be another \mu_0 somewhere??
\end{equation}
where $\ev$ is the easy axis direction and $K_u$ is the (material dependant) first order anisotropy constant. The first order approximation is valid when the anisotropy constants are such that $K_u = K_1 >> K_2$, which is the case in most materials.\cite{Kronmuller2003} The effective field corresponding to this case is
\begin{equation}
  \Hca = \frac{2 K_u}{\mu_0^2 M_s^2} (\Mv \cdot \ev) \; \ev.
  \label{eq:Hca}
\end{equation}
...
However equation~\eqref{eq:38} implies that the precession motion is undamped, i.e. no energy is lost and the magnetisation continues to precess forever. This is obviously not the case so an empirical damping term is added:
\begin{equation}
  \label{eq:LL}
  %\tag{LL}
  \dMdt = - \gymagc \Mv \times \Hv - \frac{\dampc_{\text{\tiny{L}}}}{M_s} \Big( \Mv \times (\Mv \times \Hv) \Big),
\end{equation}
where $\dampc_{\text{\tiny{L}}}$ is an experimentally determined material parameter related to the strength of the damping. Equation~\eqref{eq:LL} is referred to as the Landau--Lifshitz.\cite{Landau1935} The effects of the precession and damping terms are shown in Figure~\ref{fig:LLG-terms}.
