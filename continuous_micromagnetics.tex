...
\end{equation}
where $\dampc$ is an experimentally determined material parameter related to the strength of the damping. Equation~\eqref{eq:LL} is referred to as the Landau-Lifshitz equation. The effects of the precession and damping terms are shown in \autoref{fig:LLG-terms}.
...
\end{equation}
However this means that time scale changes when switching between the two forms (\ie the $t^*$s are different), making comparisons more difficult.
