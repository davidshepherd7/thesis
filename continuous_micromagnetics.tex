...
Equations~\cref{eq:LL,eq:Gilbert,eq:LLG} link the unknown magnetisation vector as a function of time, $\Mv(t)$, with the effective field, $\Hv$.
...
Note that there is no spatial dependence directly contained in equations~\cref{eq:LL,eq:Gilbert,eq:LLG}.
...
Combining equation~\cref{eqn:llgnd} with the definitions~\cref{eqn:nddefs} gives
\begin{equation}
   \dmdt \gymagc M_s^2 =
