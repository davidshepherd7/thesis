...
% ??ds include derivation all the way from Maxwell's equations, if so add curl above -- Andrew

When the magnetic field is produced only by magnets and not by electric currents (\ie the magnetic field is irrotational) it is possible to express the field in terms of a scalar potential, $\phim$: % \cite{Coey2010}
\begin{equation}
  \begin{aligned}
    \label{eq:Hms}
    \Hms &= - \grad \phim, \\
