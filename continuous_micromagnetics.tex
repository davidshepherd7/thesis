...
% ??ds check this in more detail, should we be using energy density? Add definition of variational derivative? -- Andrew
...
% ??ds could add better derivation or a citation? -- Andrew, check the M_s factors: should this be \mv?
...
When the rate of change of the electric field is small (\ie there are no rapidly changing electric currents, which is typically the case in micromagnetic problems \cite{??ds}) the magnetic field is well approximated by the magnetostatic equations.
The magnetostatic equations can be derived from Maxwell's equations by setting $\pd{\mathbf{E}}{t} = 0$ \cite{??ds}.

We call the resulting field the magnetostatic field, $\Hms$.
The magnetostatic field can be written in two ways: an integral form or a differential (potential) form.
...
In terms of a scalar potential, $\phim$, the magnetostatic field is: % \cite{Coey2010}
\begin{equation}
  \begin{aligned}
    \label{eq:Hms}
    \Hms &= - \grad \phim, \\
...
By far the most commonly used boundary conditions on the magnetisation $\Mv$ are \cite[178, 181]{Aharoni1996}:
\begin{equation}
  \label{eq:llg-bc}
  \Mv \times \dMdn = \zerov.
\end{equation}
