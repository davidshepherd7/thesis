...
For the Landau-Lifshitz equation~\cref{eq:ndll} we have a vector of three residuals.
\begin{equation}
  \begin{aligned}
...
\end{aligned}
\end{equation}
where we have substituted in equation~\cref{eqn:mbc} (the boundary condition on $\mv$).
...
Next the second term of equation~\cref{ll-eqn:exresterms}
\begin{equation}
...
\end{equation}
or using the notation given in equation~\cref{ll-eqn:gradvecm}
\begin{equation}
  \mxmxex_2 = \int_\magd \nabla \mv \cdot \nabla \tbf  \d\magd.
\end{equation}
...
So substituting the basis representations, \cref{ll-eq:unknowns-basis,ll-eq:47} into the residuals we obtain a spatially discretised version of the problem.
%% \begin{gather}
%%   \label{ll-res:tintro}
...
%%   - \int_{\magd} (\div (\sum_k \mv_k \sk) ) \tn \d \magd \\
