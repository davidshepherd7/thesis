...
% ??ds first draft
A simple method of dealing with the constraint is to re-normalise each value of $\mv$ after every time step.
A simple extension of this method is to re-normalise the magnetisation when the error in $\abs{\mv}$ exceeds some tolerance \cite{Fidler2000}, we refer to this method as \emph{tolerance-based renormalisation}.
Note that the computational cost of computing the errors in $\abs{\mv}$ is roughly the same as performing the re-normalisation\footnote{Both require computing $\abs{\mv}$ and performing some simple operation with it for every discrete value of the magnetisation.}, hence tolerance-based re-normalisation provides no reduction in the computational cost.
...
\end{equation}
where the final term has been added, and $\scc$ is some parameter which must be chosen.
...
For large $\scc$... small... ??ds fill in
...
This method is used by \oommf's forward Euler scheme \cite{fangor-donahue-email}.
...
% ??ds first draft
The total computational complexity of LU decomposition for a banded matrix is $\order{s^2 \nrow}$, where $s$ is the bandwidth (\ie $s$ such that the matrix entries $a_{kl} = 0$ if $\abs{k - l} > s$) \cite[236]{Iserles2009}.
As an example we consider the complexity for the system resulting from the discretisation of a PDE in two dimensions on an $a \times a$ grid by the finite element or finite difference method.
...
So the performance of a direct solver on three dimensional problems is much worse than for comparable problems in two dimensions, even if $\nrow$ remains the same.

% ??ds first draft
For general unstructured meshes the derivation would be much more complex.
