...
However a simple approximation can be given easily: for $\fv$ linear in $yv$, $t$ TR and IMR are the same, hence term I must be the same as TR in this case, a second error term is then added to $\lte^\imr$ corresponding to II for non-linear $\fv$.
...
However in terms of the accuracy of calculations the \emph{accumulated} local truncation error is what matters, so these comparisons are not the whole story.
...
This order reduction effect is a potential downside for IMR, but only when applied to specific ODEs (no such effect has been reported in micromagnetics simulations we are aware).
...
Intuitively the conservation properties of the IMR discretisation can be seen to come from the cancellation of cross terms in the inner product $\ip{\lop[\mv]}{\dmdt}$ for any symmetrical linear operator $\lop$.
More precisely:
\begin{equation}
  \begin{aligned}
    \label{eqn:imr-linop}
...
\end{equation}
Additionally the energy at time step $n$ due to the effective field operator $\hop$ can be written as\footnote{This essentially follows from the definitions of the (non-dimensionalised) energies and the effective fields given in \cref{sec:land-lifsh-gilb-normalisation}, details are given in \cref{sec:energy-field-relation}.}
\begin{equation}
  \label{eq:energy-hop}
...
For an LLG problem by semi-discretised by the finite element method $\yvdis_n$ is the list of all the discrete values that define $\mv_n$ along with any values needed for the magnetostatic calculations.
Using this notation the non-linear problem can be written as
\begin{equation}
  \text{Find $\yvdis_{n+1}$ such that } \resi(t_{n+1}, \yvdis_{n+1}, \yvdis_n, \yvdis_{n-1}, \ldots) = 0,
\end{equation}
where the number of previous time step values, $\yvdis_i$, required depends on the time integration scheme.
...
\end{equation}
in which, if the Taylor expansion is valid so that we can discard the higher order terms, the error is \emph{squared} after each iteration (\ie convergence is quadratic).
...
\begin{equation}
  \norm{\resi(\nlit{\yvdis}{i+1})}_{\infty} < \norm{\resi(\nlit{\yvdis}{0})}_{\infty} \nrtol.
\end{equation}
If the residual expression is well normalised, \ie has initial magnitude of $\order{1}$, then the two are roughly equivalent.
...
There are two general classes of method: direct methods and iterative methods.
Direct methods are very general but typically computationally expensive in both memory and time.
Effective iterative methods typically must be developed for a specific problem but can be much more efficient or even achieve optimal scaling (\ie of computational complexity $\order{\nrow}$ in time and memory).


\subsection{Direct methods}
\label{sec:direct-methods}

Direct methods for the solution of linear systems construct exact solutions to linear systems of equations using methods based on Gaussian elimination.
...
In the context of PDEs: for three dimensional problems there are significantly more non-zero entries in each row than in two dimensions (due to the increased connectivity of discrete elements).
Hence the performance of a direct solver is much worse than for similar problems in two dimensions, even if $\nrow$ remains the same.
...
These methods are based on finding an approximation to the solution, $\xv$, in the sequence of Krylov subspaces of the matrix $\Am$:
\begin{equation}
  \krylov_m(\Am, \yv) = \spanop \set{\yv, \Am \yv, \Am^2\yv, \ldots, \Am^m\yv}.
  \label{eq:96}
\end{equation}
Note that computing a basis for the next Krylov subspace $\krylov_{m+1}$ only requires a matrix vector product with the matrix $\Am$ applied to a basis vector of $\krylov_m$ (although in practice the simple basis in \cref{eq:96} is modified to improve the properties of the method).
Hence constructing the subspace is computationally cheap for a sparse matrix $\Am$.
At each iteration a new approximation to the solution in the space $\krylov_{m+1}$ is found by minimising some easy to calculate form of the error in the direction given by the new Krylov basis vector.
...
1) Find a new direction in the $m$-th Krylov subspace which is ``orthogonal'' to all previous directions.
...
This limitation of GMRES can be avoided by restarting the solve after some number of steps with the current approximation as the new initial guess, or by using an alternative Krylov solver such as BiCGStab \cite[172]{HowardElmanDavidSilvester2006}.
...
So for optimal performance we often instead solve a modified system of the form
\begin{equation}
  \begin{aligned}
    \Bm \xv &= \cv, \quad \Bm = \precond^{-1} \Am, \quad \cv = \precond^{-1} \bv,
  \end{aligned}
\end{equation}
where $\precond$ is a \emph{preconditioner} which is chosen such that the the matrix $\precond^{-1} \Am$ has the required eigenvalue and eigenvector properties for rapid convergence.
Since the preconditioner is applied at every step of the Krylov solve it should be computationally cheap to apply.
...
In the next section we briefly discuss approaches for such preconditioners.
...
As mentioned above the preconditioning of Krylov solvers is a critical component in obtaining good performance, but it also requires a highly problem dependant approach.

There are a number of ``general purpose'' preconditioners which have some positive effect on the convergence of the Krylov solver for most linear systems.
...
The simplest way to limit the fill-in is to only $n$ ``generations'' of fill-in, \ie with $n=0$ no fill-in is allowed, with 1 only the original matrix elements are allowed to generate new elements, etc.).
...
This involves the division of the matrix $\Am$ into blocks corresponding to the different physical degrees of freedom.
...
For example, by removing or eliminating blocks the preconditioner can be solved by inverting the diagonal blocks and using block back/forward substitution.
...
In \cref{sec:solution-strategies} we introduce such a preconditioning approach for the solution of the LLG-magnetostatics system.
...
% ??ds Should I mention how to solve mass matrices? -- diagonal preconditioning? \cite{Wathen1987a}
...
The lowest frequency errors can be well represented on a very coarse grid (\ie a very small linear system), and so can be cheaply corrected for using a direct solve.

Typical multigrid solvers work use something similar to the following algorithm:
\begin{itemize}
\item Start with initial guess on the real mesh
\item Apply a few iterations of a relaxation method
...
\item Apply a few iterations of a relaxation method \\
  $\vdots$
\item End with solution on the real mesh
\end{itemize}
The details of the relaxation method used, the projection between meshes, the selection of coarse meshes, the exact order/number of repeats of the above cycle etc. all depend on the exact variant of the multigrid method used.

It turns out that for certain problems multigrid solvers can achieve computation times which scale as $\order{N \log N}$, \ie their performance does not severely degrade as the number of nodes in the mesh is increased.
The memory usage of multigrid methods is around $\order{N}$, depending on the method used to obtain the coarse meshes.
...
This makes them useful for many problems because Laplace operator is in some way the most important part of many PDEs, since it contains the highest derivatives.

Standard multigrid methods make use of the geometric properties of the mesh underlying the elliptical PDE, which makes them difficult to apply to unstructured meshes
...
These methods use information purely from the matrix itself to construct ``coarse'' versions of the matrix based on the strength of the coupling between matrix elements.
...
In all ``real world'' models using implicit time integration we know of the Newton-Raphson method is used for linearisation.
...
However the convergence proofs attainable with a fixed point iteration may be useful in some extreme cases, such as when the solution undergoes ``blow-up'' \cite{Bartels2006}.
