...
\subsubsection{The self-correcting modified LLG}
\label{sec:sc-llg}

instead solve the equation
\begin{equation}
  \label{sc-ll}
  \dmdt = \mxh + \mxmxh + \scc \mv \bigb{1 - \abs{\mv}^2}.
\end{equation}
...
This additional term will push the magnetisation length back towards $\abs{\mv} = 1$.
Similarly if $\abs{\mv} < 1$ there is a small additional change in the direction of $+\mv$.
