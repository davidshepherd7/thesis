...
??ds what do I mean by large for a matrix? This occurs when $\pd{\fv}{\yv}\evalat{\thf{}}$ is large, \ie when the error in $\ffv{t, \yv}$ due to a small error in $\yv$ is large.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
where $\jac(\nlit{\yvdis}{0})$ is the Jacobian matrix of the residual with respect to $\yvdisc$ evaluated at $\nlit{\yvdis}{0}$.
So
\begin{equation}
  \begin{aligned}
    \label{eq:49}
