...
These calculations involve solving some form of the LLG equation \eqref{eq:LLG} with effective fields determined by energy derivatives as discussed in \autoref{sec:cont-micromag}.
...
Alternatively, if the equivalent potential formulation \eqref{eq:Hms}, \eqref{eq:cont-phi-bound} is used then the magnetostatic field calculation simply requires solving additional PDEs.

It should be noted that not all methods fit into this framework of applying separate methods for each part of the problem, see \autoref{sec:advanced-lin} for one example.
...
However it is often simpler to construct a macrospin model than to use the more general methods described in \autorefs{sec:sd-finite-diff-meth, sec:sd-finite-elem-meth}.
...
After the application of one of the spatial discretisations described above a PDE is typically reduced to the form \eqref{eq:45}, this sometimes is called a \emph{semi-discretised} PDE.

The time integration methods discussed here all bear a strong similarity to the finite difference method discussed in \autoref{sec:sd-finite-diff-meth}, except that the independant variable discretised is time instead of space.
...
\item \textbf{Stability} -- Roughly speaking a scheme is stable if the approximate solution does not ``blow up'' (\ie become catastophically inaccurate, infinite, oscillate in non-physical ways, \ldots) for sufficiently large time steps (see \autoref{sec:A-stability} for a more rigorous description).
...
For example, certain important properties of the Landau-Lifshitz-Gilbert equation discussed in \autoref{sec:prop-cont-llg}.
...
However all explicit time integration schemes suffer from issues of limited stability for some problems, and so are forced to use time step sizes much smaller than would be required for reasons of accuracy (see \autoref{sec:A-stability} for details).
...
In \autoref{cha:stiffn-llg-equat} we investigate stiffness in micromagnetics problems.
...
All three are implicit schemes for reasons of stability, as mentioned in \autoref{sec:explicit-vs-implicit-schemes}.

\emph{Trapezoid rule} (TR) is the average of the forward and backward Euler methods from \autoref{sec:explicit-vs-implicit-schemes}:
\begin{equation}
...
\end{equation}
Note that for cases where $\fv$ is linear in both $t$ and $\yv$ IMR, \eqref{eq:imr-definition}, is equvalent to TR, \eqref{eq:tr-definition}.
In general the properties of the two methods are similar.

IMR is trivial to implement in any existing implicit solver: take a step of BDF1, \eqref{eq:bdf1-definition}, using $\dtn^\bdfo = \dtn^\imr/2$ then update via
\begin{equation}
  \begin{aligned}
...
\end{equation}
The difference is that the global error includes any error accumulated over previous time steps (\ie the exact values $\yv(t_n)$ in \eqref{eq:60} are replaced by their approximations).
Note that because of this accumulation the global error will typically be larger than the LTE.
For example the global error of IMR expressed in the same form as \eqref{eq:trunc-start} is
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:taylorn}
\end{equation}
Substituting \eqref{eq:taylornp1} and \eqref{eq:taylorn} into \eqref{eq:trunc-start} gives\footnote{If we had chosen to Taylor expand about $t_n$ there would be an additional term in $\yv_n''$ in \eqref{eq:trunc-mid}.}
\begin{equation}
...
There are two parts to \eqref{eq:trunc-mid}: the first term (I) is fairly standard for second order time integration schemes (see \eg \autoref{cha:trunc-errors}).
...
An additional condition required in the derivation is that ??ds the magnitude of the eigenvalues of $\dtn\dfdyhf$ are less than one, the implications of this are discussed in \autoref{sec:order-reduction}.
