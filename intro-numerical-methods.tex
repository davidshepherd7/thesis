...
??ds needs lots of work
\begin{itemize}
\item Comp. time prop to accuracy as ``low'' as possible
\item Robust vs changes in problem
\end{itemize}

\begin{itemize}
\item Time step size dictated only by the physics (and required accuracy)
  Ideally the fineness of the time discretisation should be determined only by what is needed to accurately resolve the physics, not by numerical properties.
  Also the required fineness should be determined automatically by the algorithm.

\item Computation time scales linearly as desired accuracy increases.
  Ideally the overall computation time should increase as little as possible as the desired accuracy increases.
  Two effects: scales as O(N), where N is ``size'' of spatial/time discretisation (easy for time, hard for space).
  Order: high as possible so that more points = more accuracy... we don't do this (2nd order only)

Related to (1): some methods greatly increase time cost when increasing spatial accuracy.

\item Can handle ``interesting'' geometries without extortionate cost:
  Ideally our method should handle any geometry without issue (this is especially important for the study of magnetic nanoparticles, whose properties are largely determined by their shape.
  
\item Robustness
  Ideally the user should not need extensive knowledge about the internals in order to get accurate results.
  In other words there should be few edge cases or ``interesting'' suprises when applying the method to different physical problems.
  
  Ideally there should be mathematical evidence that all parts of the solver will ``work'' on any given problem (convergence proofs...).

  Also should obey qualitative physical properties: $\abs{\mv}$ fixed, energy conservation, no spurious damping.


\item Simplicity (mathematical, implementation and interface to user):
  Ideally all parts of a computational solver would be understandable by anyone with a reasonable background in mathematics.
  Unfortunately this is difficult to maintain while providing the other properties
...
Additionally  such methods are only accurate for materials where the magnetic moment is localised to an atom, which is not always the case \cite{??ds}.
...
The main advantage of the finite element method is that it can cheaply and accurately (compared to the finite difference method) approximate non-trivial geometrical features by an using an appropriate mesh of polygonal elements.
...
In \thisref{sec:magn-field-calc} we use the term ``magnetic charges'' to mean the macrospins/cells/nodes as appropriate for the spatial discretisation.
??ds replace magnetic charges by discrete points or something...
...
??ds refer to original equations 
...
Hence other techniques must be used, such techniques are the subject of the rest of \thisref{sec:numer-meth-micr}.
...
A-stability is a good way to classify the suitability of a time integrator for solving stiff ODEs \cite{??ds}.
...
??ds write about energy? energy moves from one type to another when length changes so renormalisation moves energy
...
??ds Not sure these downsides are true, seems like Mansuripur88 managed to derive all the fields in polars with no conversion to cartesians, only for macrospins though
...
??ds mention not related to spurious damping
