...
As an example we can use functional iteration to solve the non-linear system resulting from the discretisation of a non-linear ODE, written as $\dydt = f(t,y)$, by the implicit midpoint rule.
...
Using this notation a non-linear problem can be written as
\begin{equation}
...
??ds other name for matrix (so that it doesn't clash with Poisson)?
...
Direct methods are very general, but are typically computationally expensive in both memory and time.
Effective iterative methods are usually problem specific but can be much more efficient and can even achieve optimal scaling (\ie of computational complexity $\order{\nrow}$ in time and memory).


\subsection{Direct methods}
\label{sec:direct-methods}

Direct methods for the solution of linear systems seek exact solutions to linear systems of equations using methods based on Gaussian elimination.
...
The problem is that the factors of a typical sparse matrix are significantly more dense than the original matrix, \ie there is some amount of fill-in.
...
Hence the performance of a direct solver is much worse than for comparable problems in two dimensions, even if $\nrow$ remains the same.
...
However, for the solver to be efficient the number of iterations should be much smaller than $\nrow$, and preferably constant with respect to $\nrow$.
...
This makes them useful for a number of problems because Laplace operator is an important part of many PDEs.
