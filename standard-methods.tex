...
The method that this thesis focuses on (for reasons that will become clear later) is the implicit midpoint rule (IMR).
It is given by
\begin{equation}
...
\subsection{??ds Why use the implicit midpoint rule}

??ds clean all this up and merge
...
\item Proven to converge when applied to FEM with nodal quadrature scheme.
\end{itemize}
...
The \emph{order} of a time integration scheme is ??ds
...
Linear explicit methods are never A-stable \cite{Nevanlinna1974} (``linear'' methods in this reference include all common time integration methods: Runge-Kutta, multistep, predictor-corrector, ...)\footnote{These limitations could theoretically be circumvented by using significantly more complex methods, but such questions are far beyond the scope of this thesis.}, which is the main reason for our focus on implicit methods.
...
??ds expand? merge into multigrid?

There are a number of classic iteration methods which are fairly ineffective when used directly to solve reasonably sized linear systems but which are the building blocks of other methods.

Examples are Jacobi, Gauss-Seidel and SOR methods \cite[103]{Saad2000}.

All can be in a form where each iteration is a matrix vector product.


\subsection{Multigrid methods}

Multigrid methods are an efficient class of linear solvers \cite{multigrid-tut}.
They are based on two observations: firstly that most ``simple'' iterative solvers are very effective at reducing the high-frequency (in space) error of a solution, but fail on the low frequency parts.
The second observation is that the low frequency errors can be converted to high frequency errors by solving the problem on a coarser mesh.
So by solving the problem on a sequence of meshes all frequencies of error (except the very lowest, which can be removed by a direct solve on a very coarse mesh) are reduced.

Typical multigrid solvers work using something like the following algorithm
\begin{itemize}
...
They are truely effective only for M-matrices (??ds large negative diagonal, small off diagonal?). 
This type of matrix is the result of discretising a Poisson problem, which is in some way the most important part of many elliptic PDEs.

In fact it turns out that multigrid methods are most effective when combined with another type of solver, as will be discussed below.
...
However \label{eq:phi-inf-bound}, the boundary condition on $\phim$ at infinity, is problematic.
...
Magnetostatic calculations on the resulting non-uniform mesh are typically then carried out using either FMM (fastmag) or FEM/BEM (nmag, magpar, femme, ...).
