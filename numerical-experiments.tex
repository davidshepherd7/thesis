...
In \cref{fig:mumag-ts-comparison} we show a comparison of the three time integration schemes (with Gaussian quadrature, monolithic magnetostatics and the highest spatial refinement) for field 2.
We can see that all three of the schemes give similar magnetisation values.
...
since the field is initially very large, the Jacobian will contain similarly large terms, resulting in a reduction of accuracy for IMR.
...
We focus on field 2 and monolithic magnetostatics.
In \cref{fig:mumag4-nodal-quadrature-comparison} we show the results for a range of mesh refinements and integration schemes at the time when they start to deviate.
If we assume that the results with Gaussian quadrature and the highest refinement level are the most accurate, then we can see that the use of nodal quadrature results in lower accuracy for a given refinement.

\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{{{plots/mumag4_nodal_quadrature/meanmysvstimes}}}
  \caption{Mean $y$ component of the magnetisation for the \mumag problem 4 with field 2, IMR, monolithic coupling, both quadrature schemes and varying spatial resolution.
    The number in the legend indicates $n_x/5$.}
  \label{fig:mumag4-nodal-quadrature-comparison}
\end{figure}

Varying the tolerance of the adaptive integrator ($\toltt = 10^{-3}, 10^{-4}, 10^{-5}$) has almost no effect on these results.
In addition the use of TR and BDF2 give similar results.

Hence we can conclude that the reduced spatial accuracy of nodal quadrature is responsible for the deviations seen.
