...
We also observed that the accuracy of the BDF2 method is lower than the TR or IMR methods by around one order of magnitude.
...
\label{more-mumag4-correction-1}
In \cref{fig:mumag-ts-comparison} we show a comparison of the three time integration schemes (with Gaussian quadrature, monolithic magnetostatics and $n_x= 100$) for field 2.
We can see that all three of the schemes give very similar magnetisation values.
However, IMR takes significantly smaller time steps during the initial relaxation phase ($t \sim -250$). This is likely due to the order reduction effect (see \cref{sec:order-reduction}):
since the field is initially very large, the Jacobian will contain large terms resulting in a reduction of accuracy for IMR.
The adaptive time integration scheme detects and accounts for this effect by reducing the time step size.
\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{{{plots/mumag4_ts_comparison/meanmxsvs-meanmysvs-meanmzsvs-dtsvstimes}}}
  \caption{Evolution of the mean magnetisation for the \mumag problem \#4 with field 2
    using various time integration schemes,
Gaussian quadrature,
monolithic coupling,
and $n_x=100$.
  }
  \label{fig:mumag-ts-comparison}
\end{figure}
...
\subsection{Effect of nodal quadrature scheme on spatial convergence}
\label{more-mumag4-correction-2}

In this section we compare the results when using nodal quadrature and Gaussian quadrature.
We focus on the case of field 2 and the use of monolithic coupling.
In \cref{fig:mumag4-nodal-quadrature-comparison} we show the mean $y$ component of the magnetisation for a range of mesh refinements and integration schemes around the time when the results begin to deviate from each other.
Varying the tolerance of the adaptive integrator ($\toltt = 10^{-3}, 10^{-4}, 10^{-5}$) or the time integration scheme has almost no effect on these results.
Hence we can assume that the results with Gaussian quadrature and the highest refinement level are the most accurate.
Based on this we can see that the use of nodal quadrature results in lower accuracy for the same level of spatial refinement.

Therefore we conclude that nodal quadrature can have a negative effect on the overall accuracy of the scheme.
However, recall that our implementation of IMR with magnetostatics does not conserve energy (see \cref{sec:geom-integr-prop}).
So these results are not necessarily representative of the accuracy of an energy-conserving implementation.

\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{{{plots/mumag4_nodal_quadrature/meanmysvstimes}}}
  \caption{Mean $y$ component of the magnetisation for the \mumag problem 4 with field 2 solved using IMR, monolithic coupling, both quadrature schemes and varying spatial resolution $n_x$.
    The number in the legend indicates $n_x/5$.}
  \label{fig:mumag4-nodal-quadrature-comparison}
\end{figure}
...
In terms of memory usage the relationships are much simpler: neither the number of time steps or Newton iterations affects the memory used.
