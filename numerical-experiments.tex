...
??Ds make sure you refer to \cref{sec:linear-systems-probl-spec} for solver parameters
...
The methods which do not naturally conserve $\abs{\mv}$ (TR and BDF2 methods, and IMR with Gaussian quadrature) are run both with and without re-normalisation of the magnetisation.
...
\begin{equation}
  \begin{aligned}
    \mu_0 \happ_1 = [-24.6, 4.3, 0.0] \text{mT}, \\
    \mu_0 \happ_2 = [-35.5, -6.3, 0.0] \text{mT}, \\
  \end{aligned}
...
This asymmetry be expected to cause issues with energy conservation because the symmetry property \cref{eqn:imr-linop} no longer holds and so the energy property \cref{eq:orth-energy} cannot be derived.
Collocation based discretisation approaches, as used in our discretisation of the BEM operator, do not preserve the symmetry of the underlying operators and this is expected to be the cause of the asymmetry.
In contrast a symmetric discrete BEM operator could be obtained by applying a symmetric Galerkin BEM discretisation \cite{Bonnet2003} \cite[75]{Wrobel2002} to the formulation discussed in \cref{sec:hybr-finit-elem}.
...
To counteract this a higher level of fill-in could be used with nodal quadrature.
...
This issue is likely due to the asymmetry of the discrete BEM operator and could be avoided by using an alternative formulation of the BEM problem.
