...
% ??ds Hans wants to add note that this is what nmag, and probably magpar + femme do, find citations?!
...
The major difficulty is that the system contains a dense block, meaning that any operations (\eg LU decomposition, multiplication) on J will be significantly slower than for a sparse Jacobian.\footnote{Dense matrix multiplication takes $\order{\nrow^2}$ operations, while LU decomposition takes $\order{\nrow^3}$ operations \cite{??ds Iserles? doulble check power}.}
...
% ??ds not sure this is correct actually
...
\begin{figure}
  ...
  \caption{The test problem used for the linear solvers in the state at time $t=0$.
    ...
    % ??ds Add axis labels in the plot -- Andrew
  }
...
%??ds lots to do in this section, need to replace plots by means + stddev for one thing.
...
% ??ds Not really demonstrated this, remove statement?  -- Andrew
