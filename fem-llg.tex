...
% ??ds use stupid normal function space notation with a million subscripts?
% ??ds switch to \mdisc for nodal values
...
The method of weighted residuals is the first step in a variety of numerical methods including the finite element method and the spectral method.
It involves replacing the initial PDE with an equivalent but more flexible version. ??MH
...
??MH specify that as N goes to inf need Vh goes to V.
...
\begin{equation}
  \label{eq:34}
  V_{S}^{h} = \setst{\phimh }{ \phimh = \sum_{l=0}^{N_l}\phim_l \sbf_l,\, \phim_l \in \real}
  \cap \Dfs
\end{equation}
...
\begin{equation}
  \phimh = \sum_{l=0}^{N_l} \phim_l\sbf_l,
  \label{eq:y-spans}
\end{equation}
...
\begin{equation}
  \sum_{l=0}^{N_l} \phim_l\intui{ \sbf_l'\tbf'_{\ndi} } =\intui{ f\tbf_{\ndi} }
  \qquad n=0,\ldots,N.
  \label{eq:31}
\end{equation}
...
??MH explain again briefly what NR is...
...
??MH matthias hates the next two paragraphs, no idea why...
...
\begin{equation}
  \begin{aligned}
    \mv(\xv) &\approx \mvh = \sum_{k = 0}^{N} \sk \, \mv_k, \\
...
\begin{equation}
  \begin{aligned}
    ...
    \Am_{n,l} &= \pd{}{\phim_l} \sum_k \intd{ -(\grad \tbf_n \cdot \grad \sbf_k) \phim_{k}}, \\
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
then using the fact that $\pd{\mv_l}{\mv_{k}} = \delta_{lk} \pd{\mv_l}{\mv_l}$ we get
\begin{equation}
...
\begin{equation}
  \begin{aligned}
    ...
    \Dm_{n,l} &= \pd{}{\mv_l} \intd{  \skewm{\mvh} \cdot \bigb{ \happ - \grad \phim - \dampc \dmdth} \tbf_n}.
  \end{aligned}
\end{equation}
...
Most of the terms in the LLG residual are zero when differentiated with respect to any $\phim_l$ so the calculation is quite simple.
...
\begin{equation}
  \begin{aligned}
    \Bigg|\Bigg| \sum_l \beta_l \bigg[& \bigb{\dmdt_l + \mv_l \times \hv_l - \dampc \mv_l \times \dmdt_l} \cdot \mv_{j}\tbf_j(\xv_l) \\
