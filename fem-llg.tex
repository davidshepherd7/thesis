...
The function space $L^{2}(\magd)$ consists of all functions $f$ such that the norm $\norm{f}_\ltwo = \sqrt{\intd{\abs{f(x)}^{2}}$ is finite.
This can be thought of as the set of functions such that their squares are integrable.

The $n$-th Sobolev space, $\sob^n(\magd)$, is the space of all functions $f$ such that $f$ and all its first $n$ derivatives are in $L^2(\magd)$, \eg for real valued functions on the interval the second Sobolev space is
\begin{equation}
  \label{eq:H1}
  \sob^2([0,1]) = \setst{ \phim : [0,1] \rightarrow \mathbb{R} }{ \phim, \pd{\phim}{x}, \pd{^2\phim}{x^2} \in L^2([0,1])}.
\end{equation}
...
Another notation sometimes used for Sobolev spaces is $W^{n,p}(\magd)$, which denotes the set of functions $f$ such that $f$ and its first $n$ derivatives are in $L^p(\magd)$, \ie the extension of $\sob^n(\magd)$ to other $L^p(\magd)$ norms.
