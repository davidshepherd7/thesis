...
For the purposes of \thisref{sec:galerk-meth-llg} we will assume that suitable Dirichlet or Neumann boundary conditions have been determined, the details of the actual boundary conditions are more complex and are discussed in \cref{sec:hybr-finit-elem}.
...
A simple problem is used to illustrate these methods: solving the Poisson equation with Dirichlet boundary conditions in one dimension.
...
Setting $f = -\div \mv = -\pd{m_x}{x}$ gives the magnetostatic potential problem \cref{eq:Hms} with Dirichlet boundary conditions.
...
For ``well behaved'' functions the following weak formulation is equivalent but without these restrictions: find $\phim\in V_{s}$ such that
\begin{equation}
  -\int_{0}^{1}\phim''v\, dx=\int_{0}^{1}fv\, dx \qquad \forall v\in V_{T}.
  \label{eq:26}
\end{equation}
...
To see that the weak form is equivalent to \cref{eq:poisson1} consider the case when $f(x) + \phim''(x)$ is non-zero at a point $x \in (0,1)$, \ie when the strong form \cref{eq:poisson1} is not satisfied. 
...
\begin{equation}
  -\intui{ \phim''v } = -\Big[ \phim'v \Big]_{0}^{1} + \intui{ \phim'v' }.
  \label{eq:29}
\end{equation}
...
\begin{equation}
  -\intui{ \phim''v } = \intui{ \phim'v' }.
\end{equation}
 
Hence our problem can be reduced to the following: find 
\begin{equation}
\phim\in V_{S}'=\set{\phim \st \phim \in \sob^1(0,1),\, \phim(0) = \phim_a, \phim(1) = \phim_b },
\end{equation}
such that
\begin{equation}
