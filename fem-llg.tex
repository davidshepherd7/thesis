...
For \thisref{sec:weak-cons-absmv} we write the midpoint (in time) value at the $j$th node as $ \mvmp = (\mv_{j,n} + \mv_{j,n+1})/2$,  similarly $\hvmp = \hv((t_n+t_{n+1})/2, \xv_j, \mv_j)$ and $\dmmp = (\mv_{j,n+1} - \mv_{j,n})/\dtn$.
