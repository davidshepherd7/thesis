...
\begin{equation}
  \begin{aligned}
    ...
    \boundd_\Neu \cap \boundd_D &= \emptyset, \\
    \boundd_\Neu \cup \boundd_D &= \boundd, \\
  \end{aligned}
\end{equation}
...
Hence the boundary integral is only non-zero over $\boundd_{\Neu}$ where $\nabla \phim \cdot \nv = g_{\Neu}$, which gives
\begin{equation}
...
\begin{equation}
  \begin{aligned}
    ...
    &- (\mv \times \nabla \phi) + (\mv \times \lap \mv) - \dampc \left( \mv
...
\item It is easy to represent the Jacobian of a cross-product using the skew operator.
  ...
  \begin{equation}
    \pd{}{a_0} \skewm{\av} \cdot \bv = \begin{pmatrix}
      0 & 0 & 0 \\
      0 & 0 & -1 \\
      0 & 1 & 0
