...
In \cref{sec:intr-finite-ele-diff} we give a general introduction to the finite element method using the magnetostatic potential equation \cref{eq:Hms} in one spatial dimension as an example.
...
The linearisation of the LLG equation is handled using the Newton-Raphson method (see \cref{sec:newt-raph}).
We also derive an analytical expression for the Jacobian matrix in \cref{sec:llg-jacobian-calculation}.

Finally in \cref{sec:nodal-integration} we show how the geometric integration properties of IMR (as discussed in \cref{sec:time-discretisation}) can be extended from the strong form case to the weak form case as used in the finite element method.
...
Before we can start we need some definitions:
The function space $C^n(\magd)$ is the set of all functions $f$ such that $f$ and its first $n$ derivatives are continuous on $\magd$.

The function space $L^{2}(\magd)$ consists of all functions $g$ such that the norm $\norm{g}_\ltwo = \sqrt{\intd{\abs{g(x)}^{2}}}$ is finite, this can be thought of as the set of functions such that their squares are integrable.

The $n$-th Sobolev space, $\sob^n(\magd)$, is the space of all functions $g$ such that $g$ and all its first $n$ derivatives are in $L^2(\magd)$, \eg for real valued functions on the interval $[0,1]$ the second Sobolev space is
\begin{equation}
  \label{eq:H1}
...
In a slight abuse of notation we sometimes write the Jacobian of a vector function $\av$ (with $n$ values) with respect to the vector $\bv$ (with $m$ values) as
\begin{equation}
...
We need $V_{T}\subseteq \sob^0([0,1])$ and $V_s \subseteq \sob^2([0,1])$ in order to ensure that the integrals in \cref{eq:26} are well defined.
So as the solution space we choose
\begin{equation}
  \begin{aligned}
...
Since the boundary conditions are automatically enforced by the choice of $V_s$ we can choose test functions that are zero on the boundary, so an appropriate test function space is:
\begin{equation}
  \label{eq:28}
  \begin{aligned}
...
Since \cref{eq:26} must hold for all (sufficiently smooth) functions in $V_T$ we can choose a function which is non-zero only on $X$ and \cref{eq:26} will fail.
...
However, since we are given the value of $\phim'$, we can calculate the term $\bigs{\phim' v}_{0}^{1}$ of \cref{eq:29} directly.
...
To convert our continuous weak form to a discrete version we approximate the infinite dimensional test function space by an $N$ dimensional subset $V_{T}^{h} \subset V'_T$ such that  $V_{T}^{h} \rightarrow V'_T$ as $N \rightarrow \infty$.
Since $V_{T}^{h}$ is finite dimensional we can write
\begin{equation}
  V_{T}^{h} = \setst{\testh }{ \testh = \sum_{n=0}^{N}\alpha_{\ndi} \tbf_{\ndi},\, \alpha_{\ndi} \in \real},
  \label{eq:30}
\end{equation}
for some finite set of linearly independent test basis functions $\tbf_{\ndi}$ (with the obvious limitation on the Dirichlet boundary values).
Note that
...
Replacing $\test$ from \cref{eq:symmetric-weak-poisson} by $\tbf_{\ndi}$ gives an equivalent set of conditions
\begin{equation}
...
We introduce a matrix $\Am$ and a vector $\bv$ containing the results of these integrals:
\begin{equation}
  \begin{aligned}
    \Am_{l\ndi} &= \intui{ \sbf_l'\tbf'_{\ndi} }, \\
    b_{j} &= \intui{ f\tbf_{\ndi} }, \\
...
A good class of functions for this purpose is the polynomials: they are trivial to add, subtract and differentiate; and can be easily numerically integrated (see \cref{sec:numer-eval-integrals}).
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
This means that the coefficient of each basis function in \cref{eq:y-spans} corresponds directly to the value of the solution at the node where that test function has its maximum value.
...
\begin{figure}
  ...
  \caption{The local and global piecewise polynomial basis functions and numbering schemes for a one-dimensional finite element method.
  }
...
The method can also be generalised to higher order polynomials, although more nodes are required inside each element in order to provide enough values to fully define the polynomials, the use of higher order polynomials increases the order of accuracy.
However there is a trade-off: the resulting linear systems become more dense as the order of the polynomials is increased due to more basis functions overlapping with each other.
...
Finally we need a way to evaluate the required integrals computationally.
The most flexible way to do this to use a numerical quadrature scheme.
...
A Gaussian quadrature scheme using $n$ knots is able to exactly evaluate integrals of polynomials up to order $2n + 1$.
This is the highest possible polynomial order of exact evaluation for a quadrature scheme.

In higher dimensions the integration is not quite so simple.
Typically a quadrature scheme only applies for integration over a specific shape of element (or at least the scheme must be non-trivially modified for alternative geometries).
To get around this issue we use a coordinate transformation to a reference element \cite[29]{HowardElmanDavidSilvester2006}, \eg in two dimensions any four sided element is mapped to the square $[-1, 1] \times [-1, 1]$, meaning that standard quadrature schemes can be used.
...
\begin{equation}
J_e = \det \pd{\sv}{\xv} = \frac{1}{\det \pd{\xv}{\sv}},
\end{equation}
where we have used the notation defined in \cref{eq:jac-def} for the Jacobian of derivatives of a function.
...
Similar quadratures for triangular and tetrahedral elements exist \eg \cite{Dunavant1985,Rathod2006}.
...
We now apply the finite element method as discussed in \cref{sec:intr-finite-ele-diff}, together with the Newton-Raphson method discussed in \cref{sec:newt-raph}, to the solution of the Landau-Lifshitz-Gilbert equation.
...
A pair of functions $\mv \in (C^2(\magd))^3$, $\phim \in C^2(\magd)$ is a solution to \cref{eq:llgR,eq:phiR} if $\Rllg(\mv, \phim)=0$ and $\Rphi(\mv, \phim)=0$ for all $\xv \in \magd$.
...
If the boundary conditions are purely Neumann then this problem is actually singular: the solution can vary by an arbitrary constant.
This constant has no effect on the magnetostatic field and so the singularity can be dealt with, for example, by fixing the value of $\phim$ to zero at some point in the domain.

From \cref{eq:103} we have that the boundary condition on the magnetisation is
\begin{equation}
  \label{eq:m-bc}
...
Hence the boundary integral is only non-zero on $\boundd_{\Neu}$ where $\grad \phim \cdot \nv = g_{\Neu}$ and we have
\begin{equation}
...
To see that $\rllg(\mv, \testv) = 0\; \forall \testv \in (\sob^0(\magd))^3$ implies $\rllgv(\mv, \test) = \zerov \; \forall \test \in \sob^0(\magd)$ consider the vector test function $(\test, 0, 0)$.
Clearly $\rllg(\mv, (\test, 0, 0)) = \bigs{\rllgv(\mv, \test)}_x$, and similarly for the $y$ and $z$ cases.
To see the inverse relationship note that if $\rllgv(\mv, \test) = \zerov\; \forall \test \in \sob^0(\magd)$ then each term of every dot product in $\rllg(\mv, \testv)$ is zero, implying that the total residual is zero $\forall \testv \in (\sob^0(\magd))^3$.
...
\begin{equation}
  \begin{aligned}
    \Iex &= \intd{\lap \mv \cdot (\testv \times \mv)}, \\
      &= \intd{\lap \mv \cdot \bv}, \\
...
\begin{equation}
  \Iex = \sum_{i=1}^3 \bigs{\intb{b_i (\grad m_i \cdot \nv)} - \intd{\grad b_i \cdot \grad m_i}}.
\label{eq:75}
\end{equation}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
which is zero by the Neumann-like boundary condition on $\mv$, \cref{eq:m-bc}.
...
To avoid the requirement for tensor index notation it is helpful to define, for a vector $\av$
\begin{equation}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
Using this notation we can write \cref{eq:75} as
\begin{equation}
  \Iex = - \intd{\gradv \mv : \bigb{\gradv \testv \times \mv}}
...
This results in a finite number of conditions (the residuals) on a finite number of variables (the nodal values) and the spatial discretisation is complete.
...
\label{sec:fully-discr-resid}

Here we write the residual using the \emph{scalar} test function form because it is the form most suitable for use in a computer implementation.
...
We write $\dmdth$ for the fully discrete time derivative approximations at step $n+1$; and $\mvh$, $\phimh$ for the fully discrete approximations to $\mv$, $\phim$ as required for the time derivative.\footnote{These are given by \cref{eq:impl-bdf1,eq:impl-bdf2,eq:impl-imr,eq:imr-bdf1,eq:impl-tr} as appropriate, with the $\yv_i$ values replaced by the spatially discrete approximations defined in \cref{eq:unknowns-basis}.}
...
Note that the ``real'' implementation of the implicit midpoint rule \cref{eq:impl-imr} introduces a factor of $1/2$ in various places due to the substitution $ \yv \rightarrow \frac{\yv_{n+1} + \yv_n}{2}$.
For simplicity, and because we prefer the simpler BDF1-based implementation, we do not include this factor in the derivations below.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
then using the fact that $\pd{\mvdis_k}{\mvdis_{l}} = \delta_{lk}$ we see that the summation can be dropped, leaving
\begin{equation}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
where $\av \tensorprod \bv$ is the tensor product of $\av$ and $\bv$.
...
We now derive expressions for $\Pm$ and $\Qm$.
...
\section[Geometric integration with the FEM]{Geometric integration with the finite element method}
...
The problem is that (outside of the infinite node limit) the weak form equations enforce properties of the \emph{integral} of the solution, so with the FEM and IMR $\int_{\magd_e} \abs{\mvh} -1 \d\magd_e = 0$ is enforced rather than the equivalent nodal property.
The solution is to evaluate the integrals using a quadrature scheme which somehow directly links the nodal values with the integral values.
...
However in other finite element literature reduced integration refers to the use of a Gaussian quadrature whose order is not sufficient to evaluate the integrals exactly, typically it is used to reduce the computational cost of the quadrature.
\emph{Nodal integration} is the standard term for quadrature schemes where the nodal values are used directly in the quadrature (typically with mesh-free methods \eg \cite{Puso2008}), but we prefer the term ``nodal quadrature'' to avoid confusion with time integration.
...
\subsection{A local nodal quadrature scheme}
\label{sec:local-nodal-integr}

In order to regain conservation properties in a weak-form-based method we introduce a nodal quadrature scheme as used by Bartels et. al. \cite{Bartels2006}:
\begin{equation}
...
As an additional benefit this can greatly simplify the calculations since no interpolation of the values to the knots (integration points) is needed.

We now need to derive suitable values for the $\beta_l$.
To do so we represent the quadrature scheme as an integral of an appropriate interpolating polynomial (in fact the interpolating polynomial defines the quadrature scheme), then rearrange the equation to find the weights (see, \eg \cite[480]{Kincaid2002}).
...
A set of quadrature-interpolation-polynomials which give the nodal quadrature property \cref{eq:nodal-integration} are the test basis functions.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
The gradient terms are constants ($m_i$ and $\tbf_n$ are linear polynomials so their first derivatives are constant), hence the exchange contribution is integrated exactly.
...
\begin{equation}
  ...
  \label{eq:82}
\end{equation}
for all $\testv \in (\ts)^3$ where $\sumbasiscoeff = \sumbasiscoeff(\testv) = \sum_j  \abs{c_{x,j}} + \abs{c_{y,j}} + \abs{c_{z,j}}$ is the sum of all coefficients in the basis representation of $\testv$.
...
We also require that $\happ$ can be written as a linear combination of basis functions.
...
\begin{equation}
  ...
  \label{eq:46}
\end{equation}
After substituting in the IMR approximations and using a midpoint approximation for the applied field in \cref{eq:46} the expression inside the norm coincides with \cref{eq:54} of the derivation of the energy loss property for the strong form of the LLG.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
Finally, by using the triangle inequality\footnote{The standard triangle inequality is $\norm{x + y} \leq \norm{x} + \norm{y}$, by choosing $y=-a$ we see that $\norm{x - a} \leq \norm{x} + \norm{-a} = \norm{x} + \norm{a}$ giving the form used here.}, we have
\begin{equation}
  \begin{aligned}
    &\norm{ \frac{\e_{n+1} - \e_n}{\dtn} + \dampc \ltnorm{\frac{\mvh_{,n+1} - \mvh_{,n}}{\dtn}}^2 + \ltip{\frac{\happ(t_{n+1}) + \happ(t_n)}{\dtn}}{\frac{\mvh_{,n+1} + \mvh_{,n}}{2}} } \\
    &< \norm{\ltip{\happerror}{\frac{\mvh_{,n+1} - \mvh_{,n}}{\dtn}} }
  + \ntol \sumbasiscoeff.
  \label{eq:86}
  \end{aligned}
\end{equation}
...
However this a pessimistic bound, the actual error is not likely to be this large.
% ??ds Why is this a pessimistic bound, evidence  -- Andrew
% because it essentially assumes that none of the coefficients of the basis functions will cancel each other out, ie all positive or all negative. In reality this would only occur in extremely symmetric cases.
...
where
\begin{equation}
\ip{\av}{\bv}_\nodal = \sum_{j \in \textrm{nodes}} \beta_j \av(\xv_j) \cdot \bv(\xv_j),
\end{equation}
...
To obtain our result we examine the case where $\testv = \mvh$ (recall that $\mvh \in (\ts)^3$ because we are using identical test and solution function spaces and each component of $\mvh$ is a linear combination of solution basis functions).
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
\Cref{eq:83} can be satisfied without the requirement that $\abs{\mv_{k, n+1}} = 1$ $\forall k$.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
where $a,b,c,d$ are the values of basis functions at the knots and $\alpha, \beta, \gamma$ are constants.
...
Finally, we note that if the magnetisation is constant in space at times $t_n$ and $t_{n+1}$ then the integral condition in \cref{eq:23} \emph{is} sufficient to give conservation of the magnetisation length at nodes.



% ??ds first draft
\section{The self-correcting LLG term}
\label{sec:self-correcting-llg}

\newcommand{\Rsc}{\myvector{\mathcal{Q}}}
\newcommand{\rsc}{q}
\newcommand{\rscv}{\mathbf{q}}


In \cref{sec:sc-llg} an additional LLG term was introduced to automatically correct the magnetisation length.
To use this additional term in the finite element method the usual weak form, residual and Jacobian terms must be derived.

\subsection{Residual contribution}

% ??ds fix does this make sense for the LLG? or do we need to go to LL form?
We start from the strong residual form of the self correcting LLG \cref{eq:sc-llg}:
\begin{equation}
  \begin{aligned}
    \Rllg'(\mv, \phim) &= \dmdt + \mv \times \hv - \dampc \mv \times \dmdt - \scc \mv \bigb{1 - \abs{\mv}^2},  \\
    &= \Rllg(\mv, \phim) - \Rsc(\mv),
  \end{aligned}
\end{equation}
where
\begin{equation}
  \Rsc(\mv) = \scc \mv \bigb{1 - \abs{\mv}^2}.
\end{equation}
Clearly the derivation of discrete weak-form terms resulting from $\Rllg$ is unchanged and we only need to add an extra term to the final discrete weak-form residual to account for the self-correcting term.

Converting $\Rsc$ to weak residual form, as in \cref{sec:weak-form-residuals}, gives
\begin{equation}
  \rscv(\mv, \test) = \intd{\scc \mv \bigb{1 - \abs{\mv}^2} \test},
\end{equation}
which must be zero for all $\test \in V$, where $V$ is as required for the rest of the residual (\ie $V = \tsinf$ due to the derivative transfer from the exchange term).

The final discrete form can then be obtained, as in \cref{sec:fully-discr-resid}, by substituting in $\mvh$ and $\testh$ to give
\begin{equation}
  \rscv_n = \intd{\scc \mvh \bigb{1 - \abs{\mvh}^2} \tbf_n},
\end{equation}
which is added to $\rllgv_n$ of \cref{res:llg-discrete}.

\subsection{Jacobian contribution}

We now derive the contribution of the self-correcting term to the Jacobian.


Note that the self-correcting term has no effect on the magnetostatics blocks of the Jacobian.
