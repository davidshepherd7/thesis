...
Since the boundary conditions are automatically enforced by the choice of $V_s$ we can choose test functions that are zero on the boundary, then an appropriate test function space is:
\begin{equation}
  \label{eq:28}
  \begin{aligned}
...
This has no effect on the magnetostatic field and can be dealt with, for example, by pinning a single value of $\phim$.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
then using the fact that $\pd{\mvdis_l}{\mv_{k}} = \delta_{lk}$ we get
\begin{equation}
