...
??ds replace ``interpolated ...'' with proper name..
...
??ds Milan wants to know if we have uniqueness on m? think it comes from $|\mv|$.
...
??ds need to write the dmdt term somehow... $\dmdt_h$?

Now we calculate the Jacobian itself.
Written in terms of the skew operator the fully discrete LLG residual becomes
\begin{equation}
  \begin{aligned}
    \rllgv_n = \rllgv(\mv_h, \tbf_n) = \int_{\magd}
    & \dmdt \tbf_n + \skewm{\mvh} \cdot \left( \happ + \hca - \grad \phi - \dampc \dmdt
    \right) \tbf_n \\
...
\begin{equation}
  \begin{aligned}
    ...
    \Dm_{n,l} &= \pd{}{\mv_l} \intd{  \skewm{\mvh} \cdot \bigb{ \happ - \grad \phi - \dampc \dmdt} \tbf_n}.
  \end{aligned}
\end{equation}
...
If we write the damping term in the form of \cref{eq:68} we have $\gv(\mvh) = \dmdt$, and
\begin{equation}
  \label{eq:69}
...
\begin{equation}
  \begin{aligned} 
    \Dm_{n,l} &= \pd{}{\mv_l} \intd{  \skewm{\mvh} \cdot
      ...
      \right) \tbf_n}, \\
    &= \int_\magd \tbf_n \big( -\sbf_l\skewm{\happ} + \sbf_l\skewm{\grad \phi} + \dampc\sbf_l \skewm{\dmdt} - \dampc\sbf_l \jts \skewm{\mvh} \Idm_3
    \big) \d\magd, \\
    &= \intd{ \tbf_n \sbf_l \bigb{ \skewm{-\happ + \grad \phi 
          + \dampc \dmdt - \dampc \jts \mvh}}}.
...
Note that it is also possible (and common) to solve this the problem as two separate systems for $\mvh$ and $\phim$, more details and the advantages of each approach are discussed in \cref{sec:solution-strategies}.
...
Next $\Qm_{n,l}$ this is a $1 \times 3$ block matrix because we differentiate by the three components of $\mvh$. 
...
\begin{equation}
  \begin{aligned}
    \Bigg| \sum_l \beta_l \bigg[& \bigb{\dmdtml + \mvm_{,l} \times \hv(\tm, \xv, \mvm_{,l}) - \dampc \mvm_{,l} \times \dmdtml} \cdot \mvm_{,j}\tbf_j(\xv_l) \\
...
\begin{equation}
  \begin{aligned}
    \Bigg| \beta_j \bigg[& \bigb{\dmdtmj  + (\mvm_{,j} \times \hv(\tm, \xv, \mvm_{,j}))  - \dampc (\mvm_{,j} \times \dmdtmj)} \cdot \mvm_{,j} \\
...
After making this substitution and applying the triple product identities $(\av \times \bv) \cdot \av = 0$ and \cref{triple:-identity} we get
\begin{equation}
...
By substituting $\testv = \mvm$ into \cref{eq:weak-llg-imr} and using the triple product identities we obtain
\begin{equation}
  \label{eq:23}
  \begin{aligned}
    0 &= \ipg{\dmdtm}{\mvm}, \\
