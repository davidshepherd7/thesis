...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
This substitution can be obtained by simple algebraic rearrangements of \eqref{eq:bdf1-definition}.
...
Note that the ``real'' implementation of the implicit midpoint rule \eqref{eq:impl-imr} introduces a factor of $1/2$ in various places due to the substitution $ \yv \rightarrow \frac{\yv_{n+1} + \yv_n}{2}$, the derivations below do not include this ??ds yet?
...
% \end{aligned}
%\end{equation}
% Now it is easy to see that applying \eqref{eq:68} with $\gv = \happ + \hms$ gives us the Jacobian contribution
% \begin{equation}
%\begin{aligned}
...
In \texttt{oomph-lib} all integration is done in local coordinates as discussed in \autoref{sec:numer-eval-integrals}, so we now calculate weights applicable in for a local coordinate quadrature.
...
Starting from the IMR discretised weak form of the LLG \eqref{eq:weak-llg-imr}, after minimising the residual using \eg the Newton-Raphson method to tolerance $\ntol$ we have
\begin{equation}
...
% Combining equations~\eqref{eq:test-h} and \eqref{eq:test-dmdt} results in
% \begin{equation}
...
% \end{equation}
% With zero applied field $\hv[t, \mv] = \hv[\mv]$ is a symmetrical linear operator on $\mv$ with respect to the inner product $\intd{\av \cdot \bv}$ (see \autoref{sec:energy-field-relation} for a proof), using this property in a similar manner to \autoref{sec:proof-energy-prop} we obtain
% \begin{equation}
...
% \end{equation}
% which is the midpoint discretisation of the analytical energy loss rate given by equation~\eqref{eq:energy-decay}, exactly as derived in \autoref{sec:proof-energy-prop}.
% The extension to the case with the applied field proceeds exactly as for the strong form of the LLG discussed in \autoref{sec:proof-energy-prop}.
