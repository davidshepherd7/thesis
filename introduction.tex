...
%% In this report we first give a summary of the ``trilemma'' problem currently facing magnetic recording, and how it was last overcome by transitioning from longitudinal to perpendicular recording in \cref{sec:trilemma} and \ref{sec:long-perp-record}. We next give an overview of micromagnetic models, methods previously used and the rationale behind the specific methods chosen for this project (\cref{sec:cont-micromag} and~\ref{sec:numer-meth-micr}). We then give an introduction to two commonly used methods of spatial discretisation -- the finite difference and finite element methods in \cref{sec:intr-finite-ele-diff}). Sections~\ref{sec:galerk-meth-llg} and \ref{sec:hybr-finit-elem} give a detailed description of the application of the discretisation methods to the continuous mathematical model. \cref{sec:impl-test} gives a short discussion of the implementation details and some basic tests demonstrating that the model works as expected. Finally \cref{sec:current-progress} and \ref{sec:objectives} discuss the progress made so far and the future objectives of my project.
