...
The BDF2 algorithm used here is extremely similar to the method used by the \cvode library for stiff problems when the order is set to 2\footnote{By default \cvode allows the order of the method to vary.
...
% ??ds first draft
Finally in \cref{fig:vode-osc-example} we compare the time step selections used by \oomph's implementation of the adaptive BDF2 algorithm with \vode's implementation\footnote{\vode is an older implementation of \cvode in Fortran rather than C.
...
Despite this difference it can be seen that the amplitude and period of the oscillations in the selected time steps is identical, and the overall trend to increasing step size is very similar.
\begin{figure}[ht]
  \centering
...
% ??ds first draft
In \cref{sec:magn-renorm-meth} we compare re-normalisation after every step with two other methods used by open source micromagnetics codes (the self correcting LLG and re-normalisation only after a certain error in the magnetisation length is reached).
We find that there is no reason to prefer these alternative methods, hence the re-normalisation is performed after every step (after the selection of the next time step to avoid complicating the step selection process).
It is carried out by simply replacing $\mv_{n+1}$ by $\mv_{n+1}/\abs{\mv_{n+1}}$.
