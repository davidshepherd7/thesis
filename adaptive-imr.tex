...
% maybe could actually be other such methods, see notes in paper copy of thesis -- "general linear methods"
...
\item Adaptive BDF2, with the error estimate constructed using Milne's device and the variable-step leapfrog method (\ie explicit BDF2) \cite[715]{GreshoSani}.\footnote{This method is the default adaptive time integration scheme of \oomph and has been thoroughly tested over the past $\sim 10$ years.}
\label{corrections-bdf2-well-established-note}
\end{itemize}
...
The BDF2 algorithm used here is extremely similar to the method used by the \cvode library for stiff problems when the order is set to 2 \cite{cvode-manual}\footnote{By default \cvode allows the order of the method to vary.
...
% ??ds third draft
Finally in \cref{fig:vode-osc-example} \oomph's implementation of the adaptive BDF2 algorithm with \vode's implementation\footnote{\vode is an older implementation of \cvode in Fortran rather than C.
We use \vode rather than \cvode because it is easily callable from scipy \cite{scipy}.
...
\section[Numerical experiments with the ODE LLG]{Numerical experiments with an ODE form of the LLG}
...
In \cref{sec:magn-renorm-meth} we compare re-normalisation after every step with two other re-normalisation methods (the self-correcting LLG and re-normalisation only after the error in the magnetisation length reaches a specified tolerance).
...
We have found that the use of BDF2 results in significantly larger errors in the calculated switching time than when the TR or IMR methods are used, particularly when the precessional behaviour is not fully resolved.
\label{cpu-correction-0}
Assuming that a given level of error is required for an application this means that IMR and TR can take larger time steps (by a factor of $\sqrt{E_\bdf/E_\imr}$), resulting a proportional reduction in the computation time.
