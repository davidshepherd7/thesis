...
% In the plots:
% fix legends: locations and contents
% colourful scatters
% consistent colours?

% Check with Matthias:
% * IVP vs ODE, is it ok to use ODE for consistency? Mention that we always solve IVP ODEs?
...
The implicit midpoint rule (IMR) is a well known integration scheme for ordinary differential equations (ODEs) with favourable stability and accuracy properties (see \cref{sec:time-discretisation} or \cite[204]{HairerNorsettWanner}).
...
In \thisref{sec:adapt-impl-midp} we first discuss why the standard approaches used to estimate the local truncation error (LTE) fail, and the approach we will use.
We next derive the formulae required and discuss how a complete adaptive time step selection algorithm can be implemented using these estimates.
...
\end{equation}
The eBDF3 method is not widely known because it does not have the basic property essential for time integration \cite[365]{HairerNorsettWanner}:
\begin{equation}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
For constant time steps the result is much simpler: $ b = 3\dtn$, $c_n = -3/2$, $c_{n-1} = 3$ and  $c_{n-2} = -1/2$ which matches the scheme given in \cite[364]{HairerNorsettWanner}.

It should be noted that the calculation of the coefficients in \cref{eqn:ebdf-coeffs} could be comparatively expensive if the number of $y$-values is small ($\sim 100$ operations assuming no compiler optimisation of the multiplications).
...
If the ratio $\frac{\dtx{n+1}}{\dtn}$ is too small, \ie if the step just taken was much too large to attain the desired accuracy, then we reduce the step size by a factor of 2 and repeat the step.
...
However the eBDF3 predictor used in the adaptive scheme requires a derivative evaluation, which for derivatives which can \emph{only} be defined implicitly case requires a non-linear solve.
...
\begin{figure}
  \centering  \includegraphics[width=1\textwidth]{plots/aimr_odes_traces/strong_order_reduction-errornormsvs-dtsvs-tracevaluesvstimes}
...
\begin{figure}
  ...
  \caption{Behaviour of the global error norm with varying tolerance vs mean step size for the order reduction example. ??ds y-axis: $\max_t$ | ..|, legend: use $\dtn^2$ etc?}
...
\begin{figure}
  ...
  \caption{Plot of $\mv$ and $\dtn$ over time for the relaxing nano-sphere problem solved by adaptive IMR. ??ds show zoom of dtn oscillations}
...
??ds plot scatter of this for lots of damping, fixed step, adaptive tol, .. ?  + mention in experiments section
The conservation behaviour as a function of the ??ds is shown in \cref{fig:ml-aimr-newton} for ??ds.
 conservation behaviour of IMR does not vary significantly with changes to any parameter other than the final Newton residual.

%(plots not shown ??ds impossible to show--too many axes... could do a scatter of everything? Too many plots already in this section!).
...
\begin{figure}
  ...
  \caption{Plot of magnetisation length errors, $\abs{\abs{\mv} -1}$, over time for the relaxing nano-sphere problem solved with each of the three adaptive integrators (without re-normalisation of $\mv$). ??ds fix for no colour, remove tol in legend}
...
\begin{figure}
  ...
  \caption{Plot of magnetisation length errors, $\abs{\abs{\mv} -1}$, against ??ds ``Newton residual reached'' using IMR with a wide variety of $\dampc$, $\ntol$ and $\toltt$ parameters.}
...
\begin{figure}
  ...
  \caption{Convergence plot of the maximum error in energy for the relaxing nano-sphere problem with $\dampc = 0$ solved using BDF2 with and without re-normalisation of $\mv$. The energy error with other time integration methods is too small to be calculated. ??ds fix legend}
...
\begin{figure}
  ...
  \caption{Convergence plot of the switching time error, \cref{eq:sw-time-error}, against average time step for each method. In the legend 1/0 indicates re-normalisation/no re-normalisation.
 constant step IMR, adaptive IMR, adaptive TR and adaptive BDF2. ??ds zoom in, remove guidelines, Better legend}
...
\begin{figure}
  ...
  \caption{Convergence plots of the error in magnetisation, \cref{eq:m-sphere-error}, against average time step for each method. In the legend 1/0 indicates re-normalisation/no re-normalisation. ??ds zoom in, remove guidelines, Better legend position + text}
