...
\end{equation} 
The truncation error is proportional to $\pd{^3y}{t^3} = e^{-\beta t} (a(t) \sin(t) + b(t) \cos(t))$, so the adaptive algorithm should oscillate the step size in time while gradually increasing the step size as $e^{-\beta t}$ goes to zero.
...
\begin{figure}[h!]
  ...
  \caption{Behaviour of the maximum (over all time steps) of the absolute error with varying tolerance for the oscillatory, damped example ODE with exact solution $y(t) = e^{-0.5 t} \sin(2\pi t)$.}
